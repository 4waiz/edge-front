...
async function ensureMicPermission(){
  try{
    ...
    showError('Microphone permission is blocked. Click the mic icon in the address bar, allow access, then press Start Voice again.');
    setState(STATE.NEEDS_PERMISSION);
    return false;
  }
}

// ---- Voice (SpeechRecognition + simple VAD) --------------------------------
const voice = {
  enabled:false, rec:null, ctx:null, analyser:null, data:null, raf:0, lastFinal:'',
  ...
  async start(){
    ...
    this.rec.onend = ()=>{ /* auto-restart handled below */ };

    this._resumeListening();

    // Very small VAD for barge-in while TTS is speaking
    try{
      ...
      this.raf = requestAnimationFrame(tick);
    }catch(err){ console.warn('VAD stream error', err); }
  },
  ...
  _resumeListening(){
    ...
    try{ this.rec.start(); }
    catch(e){ setTimeout(()=>{ try{ this.rec.start(); }catch{} }, 250); }
  },
